...
public class GmailQuickstart {
    ...
    public static void main(String... args) throws IOException, GeneralSecurityException {
        ...
        for (Message message : messages) {
            ...
            Message test = service.users().messages().get(USER_ID, message.getId()).execute();

            List<MessagePartHeader> subject = test.getPayload().getHeaders();
            subject.forEach((name) -> {
                String findSubject = name.getName();
                String subjectValue = name.getValue();

                if(findSubject.equalsIgnoreCase("Subject")){
                    System.out.println("Subject :"+ subjectValue);
                }
            });
            System.out.println("Message :"+test.getSnippet());
        }
    }
}